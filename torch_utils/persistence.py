...
def _src_to_module(src):
    ...
    """
    def replace_relative_imports(src):
        """
        Replaces relative imports of the form 'from .xxx import xxx' with
        'from training.xxx import xxx' in the given source code string.

        :param src: Original source code string.
        :return: Updated source code string with replaced import statements.
        """
        import re
        pattern = r"from \.(?P<module>[a-zA-Z0-9_]+) import (?P=module)"
        replacement = r"from training.\g<module> import \g<module>"
        src_updated = re.sub(pattern, replacement, src)
        return src_updated
    ...
    if module is None:
        ...
        try:
            ...
            exec(replace_relative_imports(src), module.__dict__) # pylint: disable=exec-used
    return module
