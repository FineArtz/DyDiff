...
@persistence.persistent_class
class Transformer_RL(TransformerForDiffusion):
    def __init__(self,
                 data_dim,  # the dim of the input/output vector
                 max_length_of_trajectory=100,  # the length of the input/output vector
                 label_dim=0,
        ...
        self.max_length_of_trajectory = max_length_of_trajectory
        self.data_dim = data_dim
        super().__init__(input_dim=data_dim, output_dim=data_dim, horizon=max_length_of_trajectory, cond_dim=label_dim,
                         n_layer=n_layer, n_head=n_head, n_emb=n_emb, p_drop_emb=p_drop_emb, p_drop_attn=p_drop_attn,
                         causal_attn=causal_attn, time_as_cond=time_as_cond, obs_as_cond=obs_as_cond,
                         n_cond_layers=n_cond_layers
                         )

    def forward(self, x, noise_labels, returns=None, use_dropout=True, force_dropout=False, class_labels=None,
                augment_labels=None):
        x = super().forward(x, noise_labels, None)
        return x
...
class EDMPrecond(torch.nn.Module):
    def __init__(self,
        ...
        self.img_resolution = model_kwargs["img_resolution"] if "img_resolution" in model_kwargs else None
        self.img_channels = model_kwargs["img_channels"] if "img_channels" in model_kwargs else None
        self.label_dim = label_dim
        self.use_fp16 = use_fp16
        self.sigma_min = sigma_min
        self.sigma_max = sigma_max
        self.sigma_data = sigma_data
        self.rl_setting = model_kwargs["rl_setting"] if "rl_setting" in model_kwargs else False
        self.model = globals()[model_type](label_dim=label_dim, **model_kwargs)
...
class MLP(torch.nn.Module):
    def __init__(self,
        ...
        self.act = getattr(torch.nn.functional, activation) if activation else identity
        self.out_act = getattr(torch.nn.functional, out_activation) if out_activation else identity
...
#----------------------------------------------------------------------------
# MLP that concats the input first

@persistence.persistent_class
class FlattenMLP(MLP):
    def forward(self, *x, **kwargs) -> torch.Tensor:
        flat_x = torch.cat(x, dim=1)
        return super().forward(flat_x, **kwargs)
...
class BNN(torch.nn.Module):
    def __init__(self,
                ...
                output_dim: int,
                hidden_dims: List[int],
                activation: Callable = silu,
                out_activation: Callable = identity,
                num_nets: int = 1,
                **kwargs,
    ):
        super().__init__()
        ...
        self.layers = torch.nn.ModuleList()
        self.layers.append(EnsembleLinear(input_dim, hidden_dims[0], num_nets))
        for i in range(len(hidden_dims) - 1):
            self.layers.append(EnsembleLinear(hidden_dims[i], hidden_dims[i + 1], num_nets))
        self.layers.append(EnsembleLinear(hidden_dims[-1], output_dim, num_nets))

        self.max_log_var = ptu.from_numpy(np.ones([1, output_dim // 2]) / 2.0)
        self.min_log_var = ptu.from_numpy(-np.ones([1, output_dim // 2]) * 10.0)

    def forward(
        self, x: torch.Tensor, return_log_var: bool = False
        ...
        var = x[:, :, self.output_dim // 2:]
        log_var = self.max_log_var - softplus(self.max_log_var - var)
        log_var = self.min_log_var + softplus(log_var - self.min_log_var)
        if return_log_var:
            return mean, log_var
        else:
    ...
    def to(self, device):
        ...
        self.max_log_var = self.max_log_var.to(device)
        self.min_log_var = self.min_log_var.to(device)
        return self
