...
def experiment(variant):
    ...
    net_size = model_params["dyn_net_size"]
    num_hidden = model_params["dyn_num_hidden_layers"]
    num_nets = model_params.get("num_nets", 1)
    num_elites = model_params.get("num_elites", num_nets)
    num_elites = min(num_elites, num_nets)
    use_gaussian = model_params.get("use_gaussian", False)
    if use_gaussian:
        dyn_model = BNN(
    ...
    save_path = model_params.get("save_path", "./logs/dynamics_model")
    ...
    model_name = f"dynamics_model"
    if use_gaussian:
        model_name += "_gaussian"
    if normalize_obs:
        model_name += "_scaled"
    model_name += f"_nets{num_nets}_elites{num_elites}_seed{env_specs['eval_env_seed']}"
