...
class ProxyEnv(Serializable, Env):
    def __init__(self, wrapped_env, *args, **kwargs):
    ...
    def set_state(self, qpos: np.array, qvel: np.array):
...
class NormalizedBoxEnv(ProxyEnv, Serializable):
    ...
    def estimate_obs_stats(self, obs_batch, override_values=False):
        ...
        self._obs_mean_torch = ptu.from_numpy(self._obs_mean)
        self._obs_std_torch = ptu.from_numpy(self._obs_std)
        self._should_normalize = True

    def _apply_normalize_obs(self, obs):
        return (obs - self._obs_mean) / (self._obs_std + EPS) if self._should_normalize else obs
    
    def _apply_unnormalize_obs(self, obs):
        return obs * self._obs_std + self._obs_mean if self._should_normalize else obs
    
    def _apply_normalize_obs_torch(self, obs):
        return (obs - self._obs_mean_torch) / (self._obs_std_torch + EPS) if self._should_normalize else obs
    
    def _apply_unnormalize_obs_torch(self, obs):
        return obs * self._obs_std_torch + self._obs_mean_torch if self._should_normalize else obs
    ...
    def set_state(self, qpos: np.array, qvel: np.array = None):
        if qvel is None:
            obs = qpos
            qpos_len = obs.shape[-1] - self._wrapped_env.sim.data.qvel.shape[0]
        else:
            qpos_len = qpos.shape[-1]
            obs = np.concatenate([qpos, qvel], axis=-1)
        if self._should_normalize:
            obs = self._apply_unnormalize_obs(obs)

        qpos, qvel = obs[:qpos_len], obs[qpos_len:]
        if qpos.shape[-1] <= (nq := self._wrapped_env.sim.model.nq):
            qpos = np.concatenate([np.zeros(nq - qpos.shape[-1]), qpos])
        self._wrapped_env.set_state(qpos, qvel)


def get_set_state(env):
    env.origin_set_state = env.set_state
    def set_state(qpos: np.array, qvel: np.array = None):
        if qvel is None:
            obs = qpos
            qpos_len = obs.shape[-1] - env.sim.data.qvel.shape[0]
        else:
            qpos_len = qpos.shape[-1]
            obs = np.concatenate([qpos, qvel], axis=-1)

        qpos, qvel = obs[:qpos_len], obs[qpos_len:]
        if qpos.shape[-1] <= (nq := env.sim.model.nq):
            qpos = np.concatenate([np.zeros(nq - qpos.shape[-1]), qpos])
        env.origin_set_state(qpos, qvel)
    return set_state
