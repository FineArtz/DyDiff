...
def process_d4rl_dataset(env_name, save_file, gamma=0.99):
    ...
    trajectories = []
    start = 0
    data_keys = ['observations', 'actions', 'rewards', 'terminals']
    ...
    datasets = d4rl.qlearning_dataset(env)
    if 'antmaze' in env_name:
        datasets['rewards'] -= 1.0
    with open(save_file, 'wb') as f:
        pickle.dump((trajectories, datasets), f)
