...
from rlkit.logger.tabulate import tabulate
from torch.utils.tensorboard import SummaryWriter


def load_from_file(algorithm, load_replay_buffer, load_model, load_path=None, load_model_replay_buffer=False, **kwargs):
    if (load_path is not None) and (len(load_path) > 0):
        ...
        if load_model_replay_buffer:
            algorithm.model_replay_buffer = extra_data["model_replay_buffer"]
...
def set_wandb(dir_name, variant):
    if "load_params" in variant and variant["load_params"].get("load_path", None) is not None:
        print("===== Wandb resuming =====")
        run_id = variant["load_params"].get("wandb_run_id", None)
        resume = "must" if run_id else True
    else:
        resume = False
        run_id = None
    wandb.init(
